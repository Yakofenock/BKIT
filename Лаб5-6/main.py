...
goods_name = ["мясо цыпленка", "мясо гуся", "котлеты"]
vid_size = ["1 кг", "3 кг", "8кг"]
dost = ["Москва", "Иркутск", "Новосибирск"]
...
async def Chose(message: types.Message, state: FSMContext):
    ...
    for size in vid_size:
        keyboard.add(size)
    await Order.next()
    await message.answer("Сколько килограмм вы хотите купить:", reply_markup=keyboard)
...
async def Dost_chose(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"Вы заказали {user_data['chosen_good']} на {user_data['chosen_size']} в город {message.text.lower()}! \n"
        f"Спасибо за заказ!")
    await state.finish()
